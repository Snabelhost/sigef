...
class TurmaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->setDefaultOrder('designacao', 'asc');
        ...
        $col_codigo = new TDataGridColumn('codigo', 'Código', 'center', 80);
        $col_designacao = new TDataGridColumn('designacao', 'Turma', 'left');
        $col_curso = new TDataGridColumn('mapa_curso->designacao', 'Curso', 'left', 180);
        $col_turno = new TDataGridColumn('turno', 'Turno', 'center', 80);
        $col_sala = new TDataGridColumn('sala', 'Sala', 'center', 80);
        $col_capacidade = new TDataGridColumn('capacidade', 'Capacidade', 'center', 90);
        $col_status = new TDataGridColumn('status', 'Status', 'center', 90);
        
        $col_turno->enableAutoHide(600);
        $col_sala->enableAutoHide(700);
        $col_capacidade->enableAutoHide(800);
        
        $col_status->setTransformer(function($value) {
            $colors = ['Activa' => 'success', 'Concluída' => 'info', 'Cancelada' => 'danger'];
        ...
        $this->datagrid->addColumn($col_turno);
        $this->datagrid->addColumn($col_sala);
        $this->datagrid->addColumn($col_capacidade);
        ...
        $action_edit = new TDataGridAction(['TurmaForm', 'onEdit'], ['key' => '{id}']);
        $action_edit->setLabel(_t('Edit'));
        $action_edit->setImage('far:edit blue');
        $action_group->addAction($action_edit);
        
        $action_formandos = new TDataGridAction(['TurmaFormandoList', 'onReload'], ['turma_id' => '{id}']);
        $action_formandos->setLabel('Formandos');
        $action_formandos->setImage('fas:users green');
        $action_group->addAction($action_formandos);
        
        $action_disciplinas = new TDataGridAction(['TurmaDisciplinaList', 'onReload'], ['turma_id' => '{id}']);
        $action_disciplinas->setLabel('Disciplinas');
        $action_disciplinas->setImage('fas:book orange');
        $action_group->addAction($action_disciplinas);
        ...
        $icon_wrapper->add('<i class="fas fa-users" style="color: white; font-size: 20px;"></i>');
        
        $title_text = new TElement('div');
        $title_text->add('<h4 style="margin: 0; font-weight: 600; color: #1a3a5c; font-size: 22px;">Turmas</h4>');
        $title_text->add('<p style="margin: 0; color: #6c757d; font-size: 14px;">Turmas Registadas no Sistema</p>');
        ...
        $status_filter->addItems(['Activa' => 'Activas', 'Concluída' => 'Concluídas', 'Cancelada' => 'Canceladas', '' => 'Todas']);
        ...
        $btn_register->setAction(new TAction(['TurmaForm', 'onEdit']), 'Registar');
    ...
    public function onDelete($param)
    {
        ...
        new TQuestion('Deseja realmente eliminar esta turma?', $action);
    }
    ...
    public function Delete($param)
    {
        try {
            ...
            (new Turma($param['key']))->delete();
            TTransaction::close();
            $this->onReload();
            new TMessage('info', 'Turma eliminada com sucesso!');
        } catch (Exception $e) {
