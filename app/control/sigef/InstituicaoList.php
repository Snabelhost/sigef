<?php
/**
 * InstituicaoList - Lista de Instituições
 *
 * @version    2.0
 * @package    sigef
 * @author     SIGEF
 */
class InstituicaoList extends TPage
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        $col_sigla = new TDataGridColumn('sigla', 'Sigla', 'center', 80);
        $col_designacao = new TDataGridColumn('designacao', 'Designação', 'left');
        $col_tipo = new TDataGridColumn('tipo_instituicao->codigo', 'Tipo', 'center', 80);
        $col_telefone = new TDataGridColumn('telefone', 'Telefone', 'center', 120);
        $col_email = new TDataGridColumn('email', 'Email', 'left', 180);
        $col_ativo = new TDataGridColumn('ativo', 'Estado', 'center', 80);
        
        $col_telefone->enableAutoHide(600);
        ...
        $action_edit = new TDataGridAction(['InstituicaoForm', 'onEdit'], ['key' => '{id}']);
        ...
        $icon_wrapper->add('<i class="fas fa-university" style="color: white; font-size: 20px;"></i>');
        
        $title_text = new TElement('div');
        $title_text->add('<h4 style="margin: 0; font-weight: 600; color: #1a3a5c; font-size: 22px;">Instituições</h4>');
        $title_text->add('<p style="margin: 0; color: #6c757d; font-size: 14px;">Instituições de Ensino Registadas no Sistema</p>');
        ...
        $btn_register->setAction(new TAction(['InstituicaoForm', 'onEdit']), 'Registar');
    ...
    public function onDelete($param)
    {
        ...
        new TQuestion('Deseja realmente eliminar esta instituição?', $action);
    }
    ...
    public function Delete($param)
    {
        try
        {
            ...
            $object = new Instituicao($param['key']);
            $object->delete();
            TTransaction::close();
            
            $this->onReload();
            new TMessage('info', 'Instituição eliminada com sucesso!');
        }
        catch (Exception $e)
        {
