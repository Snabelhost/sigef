<?php
/**
 * DisciplinaList - Lista de Disciplinas
 *
 * @version    2.0
 * @package    sigef
 * @author     SIGEF
...
class DisciplinaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $col_codigo = new TDataGridColumn('codigo', 'Código', 'center', 100);
        $col_designacao = new TDataGridColumn('designacao', 'Designação', 'left');
        $col_carga = new TDataGridColumn('carga_horaria', 'Carga Horária', 'center', 120);
        $col_ativo = new TDataGridColumn('ativo', 'Estado', 'center', 80);
        
        $col_carga->enableAutoHide(600);
        ...
        $action_edit = new TDataGridAction(['DisciplinaForm', 'onEdit'], ['key' => '{id}']);
        ...
        $icon_wrapper->add('<i class="fas fa-book" style="color: white; font-size: 20px;"></i>');
        
        $title_text = new TElement('div');
        $title_text->add('<h4 style="margin: 0; font-weight: 600; color: #1a3a5c; font-size: 22px;">Disciplinas</h4>');
        $title_text->add('<p style="margin: 0; color: #6c757d; font-size: 14px;">Disciplinas Registadas no Sistema</p>');
        ...
        $btn_register->setAction(new TAction(['DisciplinaForm', 'onEdit']), 'Registar');
    ...
    public function Delete($param)
    {
        try {
            ...
            (new Disciplina($param['key']))->delete();
            TTransaction::close();
            $this->onReload();
