<?php
/**
 * ProvenienciaList - Lista de Proveniências
 *
 * @version    2.0
 * @package    sigef
 * @author     SIGEF
 */
class ProvenienciaList extends TPage
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        $col_descricao = new TDataGridColumn('descricao', 'Descrição', 'left', 200);
        $col_ativo = new TDataGridColumn('ativo', 'Estado', 'center', 80);
        
        $col_descricao->enableAutoHide(600);
        ...
        $action_edit = new TDataGridAction(['ProvenienciaForm', 'onEdit'], ['key' => '{id}']);
        ...
        $icon_wrapper->add('<i class="fas fa-home" style="color: white; font-size: 20px;"></i>');
        
        $title_text = new TElement('div');
        $title_text->add('<h4 style="margin: 0; font-weight: 600; color: #1a3a5c; font-size: 22px;">Proveniências</h4>');
        $title_text->add('<p style="margin: 0; color: #6c757d; font-size: 14px;">Proveniências Registadas no Sistema</p>');
        ...
        $btn_register->setAction(new TAction(['ProvenienciaForm', 'onEdit']), 'Registar');
    ...
    public function Delete($param)
    {
        try {
            ...
            (new Proveniencia($param['key']))->delete();
            TTransaction::close();
            $this->onReload();
