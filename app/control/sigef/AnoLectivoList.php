<?php
/**
 * AnoLectivoList - Lista de Anos Lectivos
 *
 * @version    2.0
 * @package    sigef
 * @author     SIGEF
 */
class AnoLectivoList extends TPage
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        $this->setDefaultOrder('ano', 'desc');
        $this->addFilterField('ano', 'like', 'ano');
        ...
        $col_ano = new TDataGridColumn('ano', 'Ano Lectivo', 'center', 150);
        $col_inicio = new TDataGridColumn('data_inicio', 'Data Início', 'center', 120);
        $col_fim = new TDataGridColumn('data_fim', 'Data Fim', 'center', 120);
        $col_status = new TDataGridColumn('status', 'Status', 'center', 100);
        
        $col_inicio->enableAutoHide(600);
        $col_fim->enableAutoHide(700);
        
        $col_status->setTransformer(function($value) {
            $colors = ['Aberto' => 'success', 'Fechado' => 'danger', 'Planeamento' => 'warning'];
        ...
        $col_ano->setAction(new TAction([$this, 'onReload'], ['order' => 'ano']));
        
        // Dropdown action group
        $action_group = new TDataGridActionGroup('', 'fas:ellipsis-v');
        
        $action_edit = new TDataGridAction(['AnoLectivoForm', 'onEdit'], ['key' => '{id}']);
        $action_edit->setLabel(_t('Edit'));
        $action_edit->setImage('far:edit blue');
        $action_group->addAction($action_edit);
        
        $action_open = new TDataGridAction([$this, 'onAbrir'], ['key' => '{id}']);
        $action_open->setLabel('Abrir');
        $action_open->setImage('fas:lock-open green');
        ...
        $icon_wrapper->add('<i class="fas fa-calendar-alt" style="color: white; font-size: 20px;"></i>');
        
        $title_text = new TElement('div');
        $title_text->add('<h4 style="margin: 0; font-weight: 600; color: #1a3a5c; font-size: 22px;">Anos Lectivos</h4>');
        $title_text->add('<p style="margin: 0; color: #6c757d; font-size: 14px;">Anos Lectivos Registados no Sistema</p>');
        ...
        $status_filter->addItems(['Aberto' => 'Abertos', 'Fechado' => 'Fechados', 'Planeamento' => 'Planeamento', '' => 'Todos']);
        ...
        $btn_register->setAction(new TAction(['AnoLectivoForm', 'onEdit']), 'Registar');
    ...
    public function onAbrir($param)
    {
        try
        {
            ...
            $anoLectivo = new AnoLectivo($param['key']);
            $anoLectivo->abrir();
            TTransaction::close();
            
            new TMessage('info', "Ano Lectivo {$anoLectivo->ano} aberto com sucesso!");
            $this->onReload();
        }
        catch (Exception $e)
        {
    ...
    public function onDelete($param)
    {
        ...
        new TQuestion('Deseja realmente eliminar?', $action);
    }
    ...
    public function Delete($param)
    {
        try {
            TTransaction::open('sigef');
            (new AnoLectivo($param['key']))->delete();
            TTransaction::close();
            $this->onReload();
            new TMessage('info', 'Eliminado com sucesso!');
        } catch (Exception $e) {
            new TMessage('error', $e->getMessage());
            TTransaction::rollback();
        }
