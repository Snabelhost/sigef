        ...
        return $table
            ...
            ->actions([
                \Filament\Actions\ActionGroup::make([
                    ...
                    \Filament\Actions\Action::make('moverInstituicao')
                        ->label('Mover Instituição')
                        ->icon('heroicon-o-arrow-right-circle')
                        ->color('warning')
                        ->requiresConfirmation()
                        ->modalHeading('Mover Cadete para Outra Instituição')
                        ->modalDescription(fn(Student $record) => 'Transferir "' . ($record->candidate?->full_name ?? 'N/A') . '" para outra instituição.')
                        ...
                        ->form([
                            ...
                            Forms\Components\Select::make('new_institution_id')
                                ...
                                ->options(fn(Student $record) => Institution::when($record->institution_id, fn($q) => $q->where('id', '!=', $record->institution_id))->orderBy('name')->pluck('name', 'id'))
                                ->required()
                                ->searchable()
                                ->preload(),
                        ])
                        ->action(function (Student $record, array $data): void {
                            ...
                            // Atualizar a instituição do cadete
